node {
   def mvnHome
  stage('Prepare') {
      git url: 'https://github.com/141149/projectsonar.git', branch: 'main'
      mvnHome = tool 'maven'
   }
  stage ('Code Quality') {
  ...
  stage ('Deliver & Deployment') {
      sh 'curl -u admin:redhat@123 -T target/**.war "http://34.227.161.42:8080/manager/text/deploy?path=/karthik&update=true"'
  }
  stage ('SmokeTest') {
      sh 'curl --retry-delay 10 --retry 5 "http://34.227.161.42:8080/karthik"'
  }
}
